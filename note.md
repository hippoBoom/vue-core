...
可以看到 `Vue` 只能通过 new 关键字初始化，然后会调用 `this._init` 方法， 该方法在 `src/core/instance/init.js` 中定义。 传入的 options 和\$options 合并
...
function initData(vm: Component) {
  ...
  while (i--) {
    const key = keys[i];
    //对比data/methods/props中的key是否相同并抛出警告
    if (process.env.NODE_ENV !== "production") {
